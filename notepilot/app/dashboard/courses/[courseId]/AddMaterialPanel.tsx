...
import { Book, Presentation, ClipboardList, Upload, Sparkles } from 'lucide-react'
import { createClient } from '@/lib/supabase/client'
import { createBook, processBook, createLectureSlide, processLectureSlide, createExam } from './actions'

type MaterialType = 'book' | 'slide' | 'exam'
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="mx-auto px-8" style={{ maxWidth: 680, paddingTop: 48, paddingBottom: 56 }}>
        ...
        <h1 className="font-bold mb-8" style={{ fontSize: 28, color: '#e8e8e8' }}>Add File</h1>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          {/* Common: title */}
          <Field label="Title">
            <DarkInput value={title} onChange={e => setTitle(e.target.value)} placeholder={
          ...
          {type === 'slide' && (
            <Field label="Lecture Number">
              <DarkInput name="lecture_number" type="number" min={1} placeholder="e.g. 3" style={{ maxWidth: 160, ...inputStyle }} />
            </Field>
          )}
          {type === 'exam' && (
            <Field label="Exam Date">
              <DarkInput name="exam_date" type="date" style={{ maxWidth: 200, ...inputStyle }} />
            </Field>
          )}
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div
                ...
                style={{ background: '#1e1e1e', border: '1px solid #e9a84c' }}
              >
              ...
              <div
                className="flex flex-col gap-3 p-4 rounded-xl"
                style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', opacity: 0.5 }}
              >
                <div className="flex items-center gap-2">
          ...
          {uploading && (
            <p className="text-[13px] text-center" style={{ color: '#e9a84c' }}>Uploading… please wait.</p>
          )}

          <div className="flex items-center justify-end gap-3 pt-2" style={{ borderTop: '1px solid #2e2e2e', paddingTop: 20 }}>
            <button
              type="button"
              onClick={() => router.back()}
            ...
            <button
              ...
              onMouseEnter={e => { if (!uploading && selectedFile) (e.currentTarget as HTMLElement).style.background = '#f0b85e' }}
              onMouseLeave={e => { if (!uploading && selectedFile) (e.currentTarget as HTMLElement).style.background = '#e9a84c' }}
            >
